...
$(function() {
    ...
    $('#itemsdiv').on('click', '.editcmp', function() {
        ...
        $('#questionedit').val($(this).parents('.pitem').find('.sbj').find('span').html());
        $('#commentedit').val($(this).parents('.pitem').find('.cmt').find('span').html());
        $("label[for='questionedit']").addClass('active');
        $("label[for='commentedit']").addClass('active');
        $("#edit").on('click', function() {
            var a = $('#formedit').serialize();
            $.ajax({
                url: siteurl + "adminquestion/edequestion",
                type: "POST",
                data: a,
                dataType: 'json',
                success: function(result) {
                    ...
                    if (result.id > 0) {
                        ...
                        row.find('.sbj').find('span').html($("#questionedit").val());
...
$(function() {
    ...
    $('#send').on('click', function() {
        ...
        $.ajax({url: siteurl + 'adminquestion/savequestion',
            data: data,
            type: 'POST',
            dataType: 'json',
            success: function(result) {
    ...
    $('#editfaqhead').click(function() {
        $.ajax({
            url: siteurl + 'adminquestion/loadquestion',
            type: 'POST',
            dataType: 'json',
            success: function(result) {
